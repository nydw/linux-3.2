 ...
 * Caller must hold RTNL.
 */
int fib_table_insert(struct fib_table *tb, struct fib_config *cfg)  //route_mark 路由表插入
{
    ...
    if (fa && fa->fa_tos == tos &&
        ...
        if (cfg->fc_nlflags & NLM_F_REPLACE) // 是否允许修改替换
        {
            struct fib_info *fi_drop;
            u8 state;

            fa = fa_first;
            if (fa_match) // 如果前面找到了匹配的就不用修改
            {
                if (fa == fa_match)
                    err = 0;
                goto out;
            ...
            fib_release_info(fi_drop); // 递归旧的路由信息结构的使用计数
            if (state & FA_S_ACCESSED) // 是否已经使用过
                rt_cache_flush(cfg->fc_nlinfo.nl_net, -1); // 冲刷缓存中的路由表
            rtmsg_fib(RTM_NEWROUTE, htonl(key), new_fa, plen, // 通过netlink向iproute2发回消息告之设置情况
        ...
        if (!(cfg->fc_nlflags & NLM_F_APPEND)) // 不允许追加到队列就使用模糊的结构
            fa = fa_first;
    }
    err = -ENOENT;
    if (!(cfg->fc_nlflags & NLM_F_CREATE))  // 不允许创建就退出函数
        goto out;
    ...
    rt_cache_flush(cfg->fc_nlinfo.nl_net, -1); // 冲刷路由缓存
    rtmsg_fib(RTM_NEWROUTE, htonl(key), new_fa, plen, tb->tb_id, &cfg->fc_nlinfo, 0); // 向iproute2发送消息，告知路由设置情况
