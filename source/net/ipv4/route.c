...
static struct dst_ops ipv4_dst_ops = { // 路由操作函数表
    ...
    .link_failure =		ipv4_link_failure, // 链接错误
...
struct rt_hash_bucket { // 路由哈希队列定义
    struct rtable __rcu	*chain; // 路由表结构队列
};
...
static struct rt_hash_bucket 	*rt_hash_table __read_mostly;  // 路由缓存用于加速路由的查找，当收到报文或发送报文时，首先会查询路由缓存，在内核中被组织成hash表，就是rt_hash_table。
...
static void rt_do_flush(struct net *net, int process_context)
{
    ...
    for (i = 0; i <= rt_hash_mask; i++) {
        ...
        if (process_context && need_resched()) // 是否需要进程调度
            cond_resched();
                           ...
                           struct rtable **result)
{
    ...
    out_dev = __in_dev_get_rcu(FIB_RES_DEV(*res)); // 取得输出设备配置结构
    if (out_dev == NULL) { // 如果没有配置结构就打印出错误信息返回
        if (net_ratelimit())
    ...
    err = fib_validate_source(skb, saddr, daddr, tos, FIB_RES_OIF(*res),
                              in_dev->dev, &spec_dst, &itag); // 检查源地址
    if (err < 0) {
    ...
    if (skb->protocol != htons(ETH_P_IP)) { // 如果不是标准IP地址
    ...
    rth = rt_dst_alloc(out_dev->dev, // 创建路由表
    ...
    rth->dst.input = ip_forward;  // 设置输入函数
    rth->dst.output = ip_output;  // 设置发送函数

    rt_set_nexthop(rth, NULL, res, res->fi, res->type, itag); // 根据查询结构设置路由表
...
static int ip_mkroute_input(struct sk_buff *skb,
                            ...
                            __be32 daddr, __be32 saddr, u32 tos)
{
    struct rtable* rth = NULL; // 初始化路由表指针为空
    int err;
    unsigned hash;

#ifdef CONFIG_IP_ROUTE_MULTIPATH
    if (res->fi && res->fi->fib_nhs > 1)
        fib_select_multipath(res);  // 优先跳转结构
#endif
    ...
    hash = rt_hash(daddr, saddr, fl4->flowi4_iif, // 计数哈希值
...
{
    struct fib_result res;
    struct in_device *in_dev = __in_dev_get_rcu(dev); // 获得设备的配置结构
    struct flowi4	fl4;
    unsigned	flags = 0;
    u32		itag = 0;
    struct rtable * rth;
    ...
    err = ip_mkroute_input(skb, &res, &fl4, in_dev, daddr, saddr, tos); // 创建用于转发的路由表
out:
    return err;
...
local_input: // 本地输入
    rth = rt_dst_alloc(net->loopback_dev,
                       IN_DEV_CONF_GET(in_dev, NOPOLICY), false); // 创建路由表
    if (!rth)
        goto e_nobufs;

    rth->dst.input= ip_local_deliver; // route_mark 本地地址的下一步的处理函数
    rth->dst.output= ip_rt_bug; // 设置输出方向的函数
#ifdef CONFIG_IP_ROUTE_CLASSID
    rth->dst.tclassid = itag;
#endif
    ...
    rth->rt_key_dst	= daddr;
    rth->rt_key_src	= saddr;
    rth->rt_genid = rt_genid(net);      // 记录随机数
    ...
    rth->dst.tclassid = itag;
#endif
    rth->rt_route_iif = dev->ifindex; //记录网络设备的id
    rth->rt_iif	= dev->ifindex;
    rth->rt_oif	= 0;
...
no_route:
    RT_CACHE_STAT_INC(in_no_route); // 递增计数
    spec_dst = inet_select_addr(dev, 0, RT_SCOPE_UNIVERSE); // 确定指定目标
    res.type = RTN_UNREACHABLE;  // 设置不可到达类型
    if (err == -ESRCH)
        err = -ENETUNREACH;
    goto local_input;
...
martian_destination:  // 目标地址错误
    RT_CACHE_STAT_INC(in_martian_dst); // 递增计数
#ifdef CONFIG_IP_ROUTE_VERBOSE
    if (IN_DEV_LOG_MARTIANS(in_dev) && net_ratelimit())
...
martian_source_keep_err:
    ip_handle_martian_source(dev, in_dev, skb, daddr, saddr); //打印错误信息
    goto out;
}
                          ...
                          u8 tos, struct net_device *dev, bool noref)
{
    struct rtable * rth;
    unsigned	hash;
    int iif = dev->ifindex; // 获取设备的ID
    struct net *net;
    int res;

    net = dev_net(dev);  // 获取设备的所属网络空间
    ...
    tos &= IPTOS_RT_MASK;  // 调整后的TOS
    hash = rt_hash(daddr, saddr, iif, rt_genid(net));  // 计算hash值

    /*
       在路由表队列中循环查找路由表，地址相同、服务类型相同、
       掩码相同、网络空间地址相同、随机数相同
    */
            ...
            rth = rcu_dereference(rth->dst.rt_next))
    {
                ...
                !rt_is_expired(rth))
        {
            ...
            RT_CACHE_STAT_INC(in_hit); // 递增命中计数
            rcu_read_unlock();
            return 0;
...
skip_cache:
     ...
     */
    if (ipv4_is_multicast(daddr)) // 如果目标地址是组播类型
    {
        struct in_device *in_dev = __in_dev_get_rcu(dev);
    ...
    res = ip_route_input_slow(skb, daddr, saddr, tos, dev); // 创建路由表
    rcu_read_unlock();
    return res;
}
...
static struct rtable *ip_route_output_slow(struct net *net, struct flowi4 *fl4) //route_mark 创建新的路由键值
{
    struct net_device *dev_out = NULL;
    __u8 tos = RT_FL_TOS(fl4); // 取得前面路由键值中的TOS值
    unsigned int flags = 0;
    struct fib_result res;
    struct rtable *rth;
    __be32 orig_daddr;
    ...
    fl4->flowi4_scope = ((tos & RTO_ONLINK) ?
                         RT_SCOPE_LINK : RT_SCOPE_UNIVERSE); // 范围

    rcu_read_lock();

    if (fl4->saddr)  // 如果指定了源地址
    {
        rth = ERR_PTR(-EINVAL);

        if (ipv4_is_multicast(fl4->saddr) ||  // 是否组播地址
                ipv4_is_lbcast(fl4->saddr) || // 是否广播地址
                ipv4_is_zeronet(fl4->saddr))  // 是否零网地址
            goto out;  // 源地址不允许是上述三种地址类型，因此返回
        ...
        if (fl4->flowi4_oif == 0 &&  // 如果没有指定发送设备
                (ipv4_is_multicast(fl4->daddr) ||  // 目标地址是组播地址
                 ipv4_is_lbcast(fl4->daddr)))  //目标地址是受限的广播地址
        {
            /* It is equivalent to inet_addr_type(saddr) == RTN_LOCAL */
            dev_out = __ip_dev_find(net, fl4->saddr, false); // 查找发送网络设备
            if (dev_out == NULL)
                goto out;
            ...
            fl4->flowi4_oif = dev_out->ifindex; // 记录上面找到的发送设备ID
            goto make_route;
        }
    ...
    if (!fl4->daddr) // 如果没有指定目标地址
    {
        fl4->daddr = fl4->saddr; // 先使用源地址作为目标地址
        if (!fl4->daddr)
            fl4->daddr = fl4->saddr = htonl(INADDR_LOOPBACK);  // 使用回接地址
        dev_out = net->loopback_dev; // 使用回接设备作为发送设备
        fl4->flowi4_oif = net->loopback_dev->ifindex; // 路由键值记录发送设备的ID
        res.type = RTN_LOCAL;   // 记录为本地地址类型
        flags |= RTCF_LOCAL;    // 增加本地路由标志
        goto make_route;
    }

    if (fib_lookup(net, fl4, &res)) {  // 调用路由函数表根据路由键值查找
        res.fi = NULL;
        if (fl4->flowi4_oif) {
            ...
            if (fl4->saddr == 0)
                fl4->saddr = inet_select_addr(dev_out, 0, RT_SCOPE_LINK);  // 源地址的查找
    ...
    if (res.type == RTN_LOCAL) { // 如果路由地址是本地类型
        if (!fl4->saddr) {
        ...
        dev_out = net->loopback_dev;  // 使用回接设备作为发送设备
        fl4->flowi4_oif = dev_out->ifindex;   // 路由键值记录发送设备的ID
        res.fi = NULL;
        flags |= RTCF_LOCAL;  // 增加本地路由标志
        goto make_route;
    }
    ...
    if (res.fi->fib_nhs > 1 && fl4->flowi4_oif == 0)
        ...
        if (!res.prefixlen &&
            ...
            fib_select_default(&res); // 查找路由信息

    if (!fl4->saddr)
        fl4->saddr = FIB_RES_PREFSRC(net, res); // 使用路由信息中的地址

    dev_out = FIB_RES_DEV(res); // 使用跳转结构的设备作为发送设备
    fl4->flowi4_oif = dev_out->ifindex;  // 记录发送设备号
...
make_route:  //到达此处已经确定了源地址，目标地址和发送设备，接着创建路由表
    ...
    if (!IS_ERR(rth)) {
        ...
        hash = rt_hash(orig_daddr, orig_saddr, orig_oif,
                       rt_genid(dev_net(dev_out))); // 计算hash值
        rth = rt_intern_hash(hash, rth, NULL, orig_oif); //由hash值确定位置插入队列
    }
...
struct rtable *__ip_route_output_key(struct net *net, struct flowi4 *flp4)
{
    ...
    hash = rt_hash(flp4->daddr, flp4->saddr, flp4->flowi4_oif, rt_genid(net)); // 确定哈希值

    rcu_read_lock_bh();
    for (rth = rcu_dereference_bh(rt_hash_table[hash].chain); rth;
            rth = rcu_dereference_bh(rth->dst.rt_next)) {
        if (rth->rt_key_dst == flp4->daddr && // 目标地址相同
                rth->rt_key_src == flp4->saddr &&  // 源地址相同
                rt_is_output_route(rth) && // 是否指定接收网络设备
                rth->rt_oif == flp4->flowi4_oif &&  //发送网络设备是否相同
                rth->rt_mark == flp4->flowi4_mark && // 掩码相同
        ...
        {
            ipv4_validate_peer(rth);
            dst_use(&rth->dst, jiffies); // 路由项记录使用时间，增加使用计数
...
slow_output:
    return ip_route_output_slow(net, flp4); // 创建新的路由键值
}
...
int __init ip_rt_init(void) // 路由初始化
{
    int rc = 0;
    ...
    if (!ip_rt_acct) // ip_rt_acct 用于路由参数统计使用
        panic("IP: failed to allocate ip_rt_acct\n");

#endif

    ipv4_dst_ops.kmem_cachep = kmem_cache_create("ip_dst_cache", sizeof(struct rtable), 0,   // 创建路由项的高速缓存，对象长度等于路由表的长度
    ...
    // 创建路由哈希桶缓存，从系统的bootmem空间分配大量的路由表空间
    rt_hash_table = (struct rt_hash_bucket *) alloc_large_system_hash("IP route cache", // 路由缓存
    ...
    expires_ljiffies = jiffies;
    schedule_delayed_work(&expires_work, net_random() % ip_rt_gc_interval + ip_rt_gc_interval);
    ...
    rtnl_register(PF_INET, RTM_GETROUTE, inet_rtm_getroute, NULL, NULL); // 获得路由的netlink
