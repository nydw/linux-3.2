...
static unsigned int fib_info_cnt; // 路由信息结构计数
...
static int fib_get_nhs(struct fib_info *fi, struct rtnexthop *rtnh,  // 修改路由信息的跳转结构
                       int remaining, struct fib_config *cfg)
{
    change_nexthops(fi)
    {
        ...
        if (attrlen > 0)
        {
            ...
            nexthop_nh->nh_gw = nla ? nla_get_be32(nla) : 0;  //取出属性数据的网关地址
#ifdef CONFIG_IP_ROUTE_CLASSID
        ...
        rtnh = rtnh_next(rtnh, &remaining); //下一个
    }
    endfor_nexthops(fi);
                        ...
                        struct fib_nh *nh)
{
    ...
    if (nh->nh_gw) {
        ...
        rcu_read_lock();
        {
            struct flowi4 fl4 = {  // 准备路由键值，查询网关的地址类型
                .daddr = nh->nh_gw,
            ...
            err = fib_lookup(net, &fl4, &res);  // 查询网关地址类型
            if (err) {
                rcu_read_unlock();
                return err;
            }
...
struct fib_info *fib_create_info(struct fib_config *cfg)
{
...
#ifdef CONFIG_IP_ROUTE_MULTIPATH
    if (cfg->fc_mp) {
        nhs = fib_count_nexthops(cfg->fc_mp, cfg->fc_mp_len); // 统计指定的跳转次数
        if (nhs == 0)
            goto err_inval;
    }
#endif
    ...
    if (cfg->fc_scope == RT_SCOPE_HOST)    // 属于本机范围
    {
        struct fib_nh *nh = fi->fib_nh;
    ...
    if (fi->fib_prefsrc)  // 如果指定了源路由地址则检查其类型
    {
...
link_it:
    ofi = fib_find_info(fi); // 检查是否存在相同的路由信息结构
    if (ofi)
    {
        fi->fib_dead = 1;
        free_fib_info(fi);
    ...
    hlist_add_head(&fi->fib_hash, &fib_info_hash[fib_info_hashfn(fi)]); // 1. 链入路由信息队列数组
    if (fi->fib_prefsrc)
    {
        struct hlist_head *head;

        head = &fib_info_laddrhash[fib_laddr_hashfn(fi->fib_prefsrc)];  // 2. 链入到地址队列数组
    ...
    change_nexthops(fi) {
        ...
        if (!nexthop_nh->nh_dev)  // 检查跳转结构是否制定跳转设备      fib_nh
            continue;

        hash = fib_devindex_hashfn(nexthop_nh->nh_dev->ifindex);  // 确定设备的hash值
        head = &fib_info_devhash[hash];
        hlist_add_head(&nexthop_nh->nh_hash, head);  // 3. 链入路由设备队列数组
