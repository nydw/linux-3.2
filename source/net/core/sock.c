                      ...
                      struct proto *prot)
{
    ...
    if (sk) {
         ...
         */
        sk->sk_prot = sk->sk_prot_creator = prot; // tcp_prot,udp_prot
        sock_lock_init(sk); //sock 使用的是自旋锁
...
int proto_register(struct proto *prot, int alloc_slab)
{
    if (alloc_slab)
    {
        ...
        if (prot->twsk_prot != NULL)
        {
            ...
            prot->twsk_prot->twsk_slab = kmem_cache_create(prot->twsk_prot->twsk_slab_name,
    ...
    list_add(&prot->node, &proto_list); // proto_list 全局静态链表
