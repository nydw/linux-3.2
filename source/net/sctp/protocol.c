...
static int sctp_v4_protosw_init(void)
{
    ...
    rc = proto_register(&sctp_prot, 1); // 注册sctp协议实例到tcp/ip协议栈
    if (rc)
        return rc;
...
SCTP_STATIC __init int sctp_init(void) // sctp_mark sctp初始化
{
    int i;
    int status = -EINVAL;
    unsigned long goal;
    ...
    sctp_bucket_cachep = kmem_cache_create("sctp_bind_bucket",
                                           sizeof(struct sctp_bind_bucket), //sctp_mark 管理 bind connect
                                           0, SLAB_HWCACHE_ALIGN,
                                           NULL);
    if (!sctp_bucket_cachep)
        goto out;

    sctp_chunk_cachep = kmem_cache_create("sctp_chunk",  // sctp_mark
    ...
    status = sctp_v4_add_protocol(); // sctp_mark 添加运输层协议
    if (status)
