                                 ...
                                 struct sock *sk)
{
    ...
    SCTP_DBG_OBJCNT_INC(chunk);  // 增加计数
...
struct sctp_association *sctp_make_temp_asoc(const struct sctp_endpoint *ep, // lgx_mark
        ...
        gfp_t gfp)
{
    ...
    asoc = sctp_association_new(ep, ep->base.sk, scope, gfp);  // make temp association
    if (!asoc)
        goto nodata;
    asoc->temp = 1;
    skb = chunk->skb;
 ...
 */
int sctp_process_init(struct sctp_association *asoc, struct sctp_chunk *chunk, // sctp_mark 处理init包
                      const union sctp_addr *peer_addr,
