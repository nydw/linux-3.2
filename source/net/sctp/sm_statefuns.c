 ...
 * The return value is the disposition of the chunk.
 */
sctp_disposition_t sctp_sf_eat_data_6_2(const struct sctp_endpoint *ep, // 确认接收data chunk
                                        const struct sctp_association *asoc,
                                        const sctp_subtype_t type,
                                        void *arg,
                                        sctp_cmd_seq_t *commands)
{
    ...
    if (!sctp_vtag_verify(chunk, asoc)) // tag 不匹配抛弃
    {
    ...
    if (!sctp_chunk_length_valid(chunk, sizeof(sctp_data_chunk_t))) // 长度不匹配,抛弃
        return sctp_sf_violation_chunklen(ep, asoc, type, arg,
                                          commands);
