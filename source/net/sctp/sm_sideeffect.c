 ...
 * good place to start.
 */
int sctp_do_sm(sctp_event_t event_type, sctp_subtype_t subtype,   // sctp_mark  状态机
               sctp_state_t state,
               struct sctp_endpoint *ep,
               struct sctp_association *asoc,
               void *event_arg,
               gfp_t gfp)
{
    ...
    state_fn = sctp_sm_lookup_event(event_type, state, subtype); // 获取状态机函数

    sctp_init_cmd_seq(&commands);

    DEBUG_PRE;

    status = (*state_fn->fn)(ep, asoc, subtype, event_arg, &commands);  //执行状态机函数   chunk_event_table
    
    DEBUG_POST;

    error = sctp_side_effects(event_type, subtype, state,  // 代码错误异常处理函数
                                ...
                                gfp_t gfp)
{
    ...
    while (NULL != (cmd = sctp_next_cmd(commands))) {
        ...
        case SCTP_CMD_SEND_PKT:
            ...
            packet = cmd->obj.ptr;
            sctp_packet_transmit(packet); // stcp_mark 将数据包转交给网络层
