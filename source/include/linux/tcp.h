...
struct tcp_sock {
    ...
    u16	tcp_header_len;	/* Bytes of tcp header to send	发送的tcp头部字节数	*/
    u16	xmit_size_goal_segs; /* Goal for segmenting output packets  分段传送的数据包数量*/
    ...
    u32	rcv_wup;	/* rcv_nxt on last window update sent	rcv_nxt在最后一次窗口更新时内容*/
    u32	snd_nxt;	/* Next sequence we send		下一个要发送的目标*/

    u32	snd_una;	/* First byte we want an ack for	第一个要ACK的字节*/
    u32	snd_sml;	/* Last byte of the most recently transmitted small packet 最近发送数据包中的尾字节*/
    u32	rcv_tstamp;	/* timestamp of last received ACK (for keepalives) 最后一次接收到的ack的时间*/
    ...
    u32	mss_cache;	/* Cached effective mss, not including SACKS  有效的mss缓存，不含sacks*/
    ...
    u8	frto_counter;	/* Number of new acks after RTO rto后的ack次数 */
    u8	nonagle     : 4,/* Disable Nagle algorithm?             是否使用nagle算法*/
     ...
     *  慢启动或阻塞控制
     */
    u32	snd_ssthresh;	/* 慢启动的起点值 Slow start size threshold		*/
    u32	snd_cwnd;	/* 发送的阻塞窗口 Sending congestion window		*/
    u32	snd_cwnd_cnt;	/*  线性计数器 Linear increase counter		*/
    u32	snd_cwnd_clamp; /* 不允许snd_cwnd超过的值 Do not allow snd_cwnd to grow above this */
    u32	snd_cwnd_used;
    u32	snd_cwnd_stamp;
    ...
    u32	rcv_wnd;	/* 当前接收窗口   Current receiver window		*/
    u32	write_seq;	/* tcp发送数据的顺序号   Tail(+1) of data held in tcp send buffer */
    u32	pushed_seq;	/* 最后送出的顺序号，需要通知窗口  Last pushed seq, required to talk to windows */
    ...
    struct sk_buff_head	out_of_order_queue; /* 超出分段规则的队列 Out of order segments go here */
