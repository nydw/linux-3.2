...
struct xt_entry_target {  // lgx_mark
    union {
        ...
        /* Total length */
        __u16 target_size; // 每一个target的大小
    } u;
...
struct xt_action_param {
    ...
    union
    {
        const void *matchinfo, *targinfo; // 上层结构match的指针
    };
    ...
    u_int8_t family;
    bool hotdrop;  // match返回0置1，立即丢弃数据报，非0，表示匹配成功
};
...
struct xt_match {
    ...
    bool (*match)(const struct sk_buff *skb, struct xt_action_param *); // 指向用该match进行匹配时的匹配函数的指针,

    /* Called when user tries to insert an entry of this type. */
    int (*checkentry)(const struct xt_mtchk_param *); // 插入新的match表项时，对新表项进行有效性检查；

    /* Called when entry of this type deleted. */
    void (*destroy)(const struct xt_mtdtor_param *); // 删除一个match的表项。
...
struct xt_target {  // lgx_mark
    ...
    unsigned int (*target)(struct sk_buff *skb,  //返回verdict
...
struct xt_table_info {
    ...
    void *entries[1];  // 规则表入口，规则存储结构 // ipt_entry 组成块的起始地址，对多cpu,每个cpu对应一个
