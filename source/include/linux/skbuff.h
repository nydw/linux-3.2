...
#ifdef NET_SKBUFF_DATA_USES_OFFSET // 64位用位移，32位用指针
...
struct sk_buff {
    /* These two members must be first. */
    struct sk_buff		*next;  //队列的下一个数据包
    struct sk_buff		*prev;  //队列的上一个数据包
    ...
    __u8			local_df:1,  // 是否允许本地数据分段
                    ...
                    nfctinfo:3;      // 数据包的连接关系
    __u8			pkt_type:3,      // 数据包的类型,单播，广播，多播等
                    fclone:2,        // 数据包克隆状态
...
static inline int skb_is_nonlinear(const struct sk_buff *skb)  // 测试一个缓冲区是否是一个分片的
{
    return skb->data_len;
}
 ...
 */
static inline int skb_linearize(struct sk_buff *skb)  // 把分片组合成一个单一的缓冲区
{
