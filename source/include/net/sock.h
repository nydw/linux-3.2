 ...
 */
struct sock {  // (tcp_sock (inet_connection_sock (inet_sock (sock))))
    /*
...
#define sk_hash			__sk_common.skc_hash  //查找哈希表时的哈希值
#define sk_family		__sk_common.skc_family //地址族
#define sk_state		__sk_common.skc_state  //连接状态
#define sk_reuse		__sk_common.skc_reuse //确定服用地址
#define sk_bound_dev_if		__sk_common.skc_bound_dev_if //绑定设备ID
#define sk_bind_node		__sk_common.skc_bind_node //链入绑定哈希表
#define sk_prot			__sk_common.skc_prot  //协议函数表 tcp_prot, udp_prot, inet_create
#define sk_net			__sk_common.skc_net   //所属网络空间
...
#define sk_rmem_alloc sk_backlog.rmem_alloc //接收队列的字节数
    int			sk_forward_alloc; //记录可用内存长度
#ifdef CONFIG_RPS
    __u32			sk_rxhash;
#endif
    atomic_t		sk_drops;
    int		    	sk_rcvbuf; // 接收缓冲的长度(字节数)
    ...
    spinlock_t		sk_dst_lock; //路由项缓存锁
    atomic_t		sk_wmem_alloc; //发送队列的字节数
    atomic_t		sk_omem_alloc; //可选的字节数
    int			sk_sndbuf; // 发送缓冲的总长度
    struct sk_buff_head	sk_write_queue; // 正在发送的数据包队列
    kmemcheck_bitfield_begin(flags);
    unsigned int		sk_shutdown  : 2, //是否关闭
                   sk_no_check  : 2,  // 是否检查数据包
                   sk_userlocks : 4,  // 用户锁
                   sk_protocol  : 8,  // 使用协议族的哪一种协议
                   sk_type      : 16; // socket 的类型 例如socket_stream
    kmemcheck_bitfield_end(flags);
    int			sk_wmem_queued; //全部数据包占用的内存计数
    gfp_t			sk_allocation; //分配模式
    int			sk_route_caps;  // 路由的兼容性标志位
    int			sk_route_nocaps;
    int			sk_gso_type;  //GSO通用分段的类型
    unsigned int		sk_gso_max_size; //用于建立通用分段的GS0的最大长度
    u16			sk_gso_max_segs;
    int			sk_rcvlowat;  // SO_RCVLOWAT设置
    unsigned long	        sk_lingertime; //停留的时间，确定关闭时间
    struct sk_buff_head	sk_error_queue;  // 很少使用
    struct proto		*sk_prot_creator; //sock 创建接口 tcp_prot,udp_prot,inet_create
    rwlock_t		sk_callback_lock;  // 为后半部处理使用的锁
    int			sk_err, //出错码
                sk_err_soft;  //持续出现的错误
    unsigned short		sk_ack_backlog;  // 当前监听到的连接数量
    unsigned short		sk_max_ack_backlog; // 在listen()中监听到的连接数量
    __u32			sk_priority;  //优先级
    struct pid		*sk_peer_pid;
    const struct cred	*sk_peer_cred; //so_peercred设置
    long			sk_rcvtimeo; // so_rcvtimeo 设置接收超时时间
    long			sk_sndtimeo; // so_sndtimeo 设置发送超时时间
    void			*sk_protinfo; // 私有区域，当不使用slab高速缓存是由协议族定义
    ...
    __u32			sk_sndmsg_off;  //发送数据块在缓存页的结尾
    ...
    u32			sk_classid;
    void			(*sk_state_change)(struct sock *sk); // sock状态改变后调用的函数
    void			(*sk_data_ready)(struct sock *sk, int bytes); // 在数据被处理后调用的函数
    void			(*sk_write_space)(struct sock *sk); // 当发送空间可以使用后调用的函数
...
struct proto {  //运输层协议结构, 例如: sctp_mark 
    ...
    struct kmem_cache	*slab;  //用于分配sock空间  proto_register
